...
### Added
- App icon: aperture-wheel concept with a cyan-glow play triangle at
  the iris center, brushed-metal outer ring, on a black background.
  Generated mipmap variants at mdpi 48, hdpi 72, xhdpi 96, xxhdpi 144,
  xxxhdpi 192 from a 1254×1254 source. Wired `android:icon` and
  `android:roundIcon` in the manifest.

## [0.1.3] - 2026-05-24

### Fixed
- **Revert v0.1.2's ad-blocking changes** — they broke video playback.
  The `&ctier=L` URL marker matched content video chunks, not just ads,
  and the generic `[class*="ytp-ad-"]` CSS hid real player buttons.
  v0.1.3 is functionally equivalent to v0.1.1 (ad blocking comes back
  to v0.1.1's level — speed-up trick + skip-button click + the v0.1.1
  CSS list — but content videos play). v0.1.4 will reintroduce shadow-
  DOM piercing alone, without the URL markers or generic CSS.
